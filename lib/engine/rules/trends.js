...
export function checkTrend1(sorted) {
  const now = Date.now();
  const msDay = 86400000;
  const thisWeek = sorted.filter(r => (now - new Date(r.timestamp)) / msDay < 7);
  const lastWeek = sorted.filter(r => { const a = (now - new Date(r.timestamp)) / msDay; return a >= 7 && a < 14; });
  ...
  const changePct = ((thisKwh - lastKwh) / lastKwh) * 100;
  return { changePct: changePct.toFixed(1), thisKwh: thisKwh.toFixed(2), lastKwh: lastKwh.toFixed(2), delta: Math.abs(thisKwh - lastKwh).toFixed(2), rising: changePct > 8 };
}
