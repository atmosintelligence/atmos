...
export function buildTrendOptimizations(trend1, trend2) {
  ...
  if (trend1?.rising) {
    opts.push(makeOpt({
      ...
      message: `Energy consumption this week (${trend1.thisKwh} kWh) is ${trend1.changePct}% higher than last week (${trend1.lastKwh} kWh), meaning an increase of ${trend1.delta} kWh. No change in occupancy patterns has been detected. This may indicate a new high-draw appliance or gradual equipment inefficiency.`,
    }));
  }
  if (trend2) {
    opts.push(makeOpt({
      group: 'Trends', severity: 'warning',
      title: 'Consistent degrading power increase',
      message: `Power usage during occupied hours has risen steadily over the past 14 days at a rate of ${trend2.slope} W per day (R² = ${trend2.r2}, forming a a strong, consistent trend). This pattern is characteristic of equipment degradation. Scheduling a maintenance check is recommended!`,
    }));
  }
  return opts;
}