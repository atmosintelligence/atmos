...
export function checkT1(readings, consecutiveEmpty = 3) {
  const events = [];
  ...
  for (let i = consecutiveEmpty - 1; i < readings.length; i++) {
    ...
    const temp = parseFloat(
      readings[i].temp_c ?? readings[i].temperature ?? 0
    );

    if (
      allEmpty &&
      (
        temp < THRESHOLDS.temperature.low ||
        temp > THRESHOLDS.temperature.high
      )
    ) {
      events.push({
...
export function checkT3(readings) {
  ...
  for (const r of readings) {
    const hum = parseFloat(
      r.humidity_pct ?? r.humidity ?? 0
    );

    if (
      hum > THRESHOLDS.humidity.high ||
      hum < THRESHOLDS.humidity.low
    ) {
      events.push({
        timestamp: r.timestamp,
        humidity: hum.toFixed(0),
        high: hum > THRESHOLDS.humidity.high
      });
    }
  }
...
export function buildHvacOptimizations(t1Events, t2Events, t3Events) {
  ...
  if (t1Events.length) {
    ...
    opts.push(makeOpt({
      ...
      message: `The room has been empty for three or more consecutive readings whilst the temperature sits at ${e.temp}°C. This is outside the comfortable 22–30°C band. If air conditioning or heating is running, it is conditioning an unoccupied space. Consider switching it off!`,
  ...
  if (t2Events.length) {
    ...
    opts.push(makeOpt({
      ...
      message: `Outdoor temperature is ${e.outdoorTemp}°C and conditions are dry, which is cooler than your room at ${e.indoorTemp}°C. Opening windows could replace air conditioning entirely and save approximately ₹${e.estimatedSaving} per hour.`,
