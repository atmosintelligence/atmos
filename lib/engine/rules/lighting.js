...
export function checkL1(readings, idleBaseline, tariff) {
  const events = [];
  for (let i = 2; i < readings.length; i++) {
    ...
    const power = parseFloat(readings[i].power_w ?? readings[i].power ?? 0);
    if (allEmpty && power > idleBaseline + THRESHOLDS.power.idleOffset) {
      ...
      events.push({ timestamp: readings[i].timestamp, vacantMins, power: power.toFixed(0), waste, idleBaseline: idleBaseline.toFixed(0) });
    }
  }
  return events;
}
...
export function checkL2(readings, idleBaseline, tariff) {
  ...
  for (const r of readings) {
    ...
    if (lux > THRESHOLDS.lux.daylight && occupied && power > idleBaseline) {
      ...
      events.push({ timestamp: r.timestamp, lux: lux.toFixed(0), power: power.toFixed(0), saving });
    }
  }
  return events;
}
...
export function buildLightingOptimizations(l1Events, l2Events, idleBaseline) {
  ...
  if (l1Events.length) {
    ...
    opts.push(makeOpt({
      group: 'Lighting', severity: 'warning',
      title: 'Lights on in empty room',
      message: `The room has been empty for ${e.vacantMins} minutes whilst drawing ${e.power} W — well above the idle baseline of ${e.idleBaseline} W. Lights or non-essential equipment appear to be running. Estimated waste: ₹${e.waste} for this period.`,
      saving: { inr: e.waste },
  ...
  if (l2Events.length) {
    ...
    opts.push(makeOpt({
      group: 'Lighting', severity: 'info',
      title: 'Daylight harvesting opportunity',
      message: `Natural light is at ${e.lux} lux — above the 400 lux comfortable working threshold. Artificial lights appear to still be on at ${e.power} W. Switching them off could save approximately ₹${e.saving} per half-hour period.`,
      saving: { inr: e.saving },
      timestamp: e.timestamp,
    }));
  }
