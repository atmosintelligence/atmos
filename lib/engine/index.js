...
import { TARIFF_DEFAULT, THRESHOLDS } from './constants.js';
...
export async function runEngine({
  ...
  const consecEmpty =
    thresholdOverrides.consecutiveEmpty
    ?? THRESHOLDS.occupancy.consecutiveEmpty;
  const phantomHours =
    thresholdOverrides.phantomLoadHours
    ?? THRESHOLDS.power.phantomEmptyHours;
  const spikeZ =
    thresholdOverrides.spikeZScore
    ?? THRESHOLDS.power.spikeZScore;
  const weekOverWeekPct =
    thresholdOverrides.weekOverWeekPct
    ?? THRESHOLDS.power.weekOverWeekPct;
  ...
  const l1Events = checkL1(
    sorted,
    idleBaseline,
    tariff,
    consecEmpty
  );
  const l2Events = checkL2(
    sorted,
    idleBaseline,
    tariff
  );
  const t1Events = checkT1(
    sorted,
    consecEmpty
  );
  const t2Events = checkT2(
    sorted,
    idleBaseline,
    tariff,
    outdoorWeather
  ...
  const p1Events = checkP1(
    sorted,
    spikeZ
  );
  const p2Events = checkP2(
    sorted,
    tariff,
    phantomHours
  );
  const p3Events = checkP3(sorted);
  const trend1 = checkTrend1(
    sorted,
    weekOverWeekPct
  );
