...
export const DEMO_DEVICE_ID_2 = 'ATM-DEMO2';

export function generateDemoReadings2(rawEntries) {
  return generateDemoReadings(rawEntries).map(r => ({
    ...r,
    device_id: DEMO_DEVICE_ID_2,
  }));
}
  ...
  return [
    {
      device_id:         DEMO_DEVICE_ID,
      owner_username:    'demo',
      installed_at:      new Date(now - 30 * 86400000).toISOString(),
      last_contacted_at: new Date(now - 86400000).toISOString(),
    },
    {
      device_id:         DEMO_DEVICE_ID_2,
      owner_username:    'demo',
      installed_at:      new Date(now - 14 * 86400000).toISOString(),
      last_contacted_at: new Date(now - 86400000).toISOString(),
    },
  ];
}